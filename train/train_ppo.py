...
def calculate_shaping_reward(env: GuanDanEnv, player_id: int, obs_before_action: dict, chosen_action: dict, action_generator: ActionGenerator) -> float:
    ...
    current_type, _ = env._check_poker_type(chosen_claim) if chosen_claim else ('pass', ())
    ...
    # =========================================================================
    # [修改] Rule H: 关键防守 (正向奖励 +0.2)
    # 场景：对手只剩 <=7 张牌，且我没有普通牌能管，只能用炸弹压制
    # =========================================================================
    if is_opponent and current_type in bomb_types:
        opponent_card_count = len(env.player_decks[last_player])
        if opponent_card_count <= 7:
            # 1. 生成所有能管上家牌的合法动作
            legal_actions = action_generator.generate_legal_actions(obs_before_action)
            
            # 2. 检查是否存在“普通牌”能管的情况
            # 普通牌定义：类型不在 bomb_types 里的所有类型 (single, pair, straight, etc.)
            can_beat_with_normal = False
            for act in legal_actions:
                c_claim = act.get('claim', [])
                if not c_claim: # 忽略 Pass
                    continue
                
                c_type, _ = env._check_poker_type(c_claim)
                if c_type not in bomb_types:
                    can_beat_with_normal = True
                    break
            
            # 3. 只有在“没有普通牌能管”（被迫炸）的情况下，才给奖励
            if not can_beat_with_normal:
                shaping_reward += 0.2

    # =========================================================================
    # Rule J: 强力压制 (正向奖励 +0.1)
    # =========================================================================
    if is_opponent and current_type in ['single', 'pair'] and current_type == last_type:
        # 假设上家出的是 > 10 的牌 (J, Q, K, A...)
        rank_idx = -1
        if last_rank_char in env.point_order:
            rank_idx = env.point_order.index(last_rank_char)
        
        # 假设 '0'(10) 的索引是 8，那么 >= 9 意味着 J, Q, K, A
        if rank_idx >= 9:
            shaping_reward += 0.1
...
def collect_rollout(env: GuanDanEnv, agents: List[RLGuanDanAgent], feature_encoder: FeatureEncoder, action_generator: ActionGenerator, replay_buffer: ReplayBuffer, num_episodes: int, device: str) -> None:
    for _ in range(num_episodes):
        ...
        while True:
            ...
            step_shaping_reward = calculate_shaping_reward(env, current_player, obs_player, chosen_action, action_generator)
...
def main() -> None:
    ...
    for update_idx in range(1, total_updates + 1):
        ...
        if update_idx % save_interval == 0:
            ...
            # === 修改这里：文件名加入 update_idx ===
            # 例如：models/ppo_checkpoint_100.pt, models/ppo_checkpoint_200.pt
            save_path = f'models/ppo_checkpoint_{update_idx}.pt' 
