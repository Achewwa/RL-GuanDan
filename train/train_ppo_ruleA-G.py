...
def calculate_shaping_reward(env: GuanDanEnv, player_id: int, obs_before_action: dict, chosen_action: dict, action_generator: ActionGenerator) -> float:
    ...
    # [新增] Rule F: 残局清小牌规则
    # 规则：剩余牌数 <= 6 时，如果还留有 <= 5 的小牌（且不成炸弹），每张扣 0.1
    # =========================================================================
    # 1. 计算出牌后的剩余手牌
    # 注意：obs_before_action['deck'] 是出牌前的，需要减去 chosen_cards
    # 这里只做简单的 id 排除（假设 chosen_cards 都在 deck 里，env 会保证这点）
    deck_after = [c for c in my_deck if c not in chosen_cards]
    
    if len(deck_after) <= 6:
        # 统计剩余牌的点数
        remaining_ranks = [utils.Num2Poker(c)[1] for c in deck_after]
        rem_counts = Counter(remaining_ranks)
        
        # 定义小牌点数: 3, 4, 5（2固定级牌）
        small_ranks = ['3', '4', '5']
        
        penalty_count = 0
        for r in small_ranks:
            count = rem_counts[r]
            if count > 0:
                # 炸弹除外：如果该点数有 >= 4 张，说明是炸弹，不扣分
                if count < 4:
                    penalty_count += count
        
        if penalty_count > 0:
            shaping_reward -= (penalty_count * 0.03)

    # =========================================================================
    # [新增] Rule G: 炸弹最小化规则
    # 规则：如果出炸弹，必须出当前能出的炸弹里最小的，不能随便用大炸弹
    # =========================================================================
    if current_type in bomb_types:
        # 1. 获取当前局面所有合法的动作
        legal_actions = action_generator.generate_legal_actions(obs_before_action)
        
        # 2. 筛选出所有合法的炸弹动作
        legal_bombs = []
        for act in legal_actions:
            c_type, c_points = env._check_poker_type(act['claim'])
            if c_type in bomb_types:
                legal_bombs.append((act, c_type, c_points))
        
        # 3. 检查是否存在比 current_action 更小的合法炸弹
        # env._check_bigger(A, A_pts, B, B_pts) 返回 True 表示 B > A
        # 我们要找的是：是否存在 B (legal_bomb)，使得 chosen (current) > B
        # 即 env._check_bigger(B_type, B_points, current_type, current_points) 为 True
        
        wasted_bomb = False
        for _, b_type, b_points in legal_bombs:
            # 如果 当前出的牌 > 某个合法炸弹，说明没用最小的
            # 注意排除自己比自己（虽然逻辑上 check_bigger 自己不大于自己，但安全起见）
            if env._check_bigger(b_type, b_points, current_type, current_points):
                wasted_bomb = True
                break
        
        if wasted_bomb:
            shaping_reward -= 0.2  # 惩罚浪费大炸弹
